...
model = Sequential(OrderedDict(
    encoder=Sequential(Linear(28 * 28, h_dim), ReLU(), Linear(h_dim, 2 * z_dim)),
...
with tqdm(range(50), desc="Epoch") as pbar:
    for epoch in pbar:
        ...
        for x, _ in loader:
            ...
            loss = binary_cross_entropy(x_, x) + (epoch * 1e-4 / 50) * p.kl()
            loss.backward()
            optim.step()
...
x_ : Tensor = model.decoder(GaussianDistribution(model.encoder(x)).mu)
...
with torch.inference_mode():
    pairs = [(z, l) for xs, ls in tqdm(loader, desc="Inference") for (z, l) in zip(GaussianDistribution(model.encoder(xs)).mu.numpy(), ls.numpy())]
...
plt.tight_layout()
plt.savefig("./figures/core_gai_vae_latent_sampling.svg")